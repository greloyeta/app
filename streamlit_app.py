...
from pizza_lib import (
    PizzaCrust, PizzaSauce, PizzaTopping, Pizza, PizzaOrder,
    PIZZA_SIZES,
    PIZZA_CRUSTS, PIZZA_SAUCES, PIZZA_TOPPINGS
)
...
for pizza_id in range(
        1,
        NUMBER_PIZZA_CONFIGURATIONS + 1
):
    ...
    pizza_toppings = streamlit.multiselect(
        ...
        [
            ...
            if topping.diameter == pizza_size
        ],
    ...
    pizza_name = streamlit.text_input(
        "Pizza Name",
        key=f"pizza_name#{pizza_id}"
    )
